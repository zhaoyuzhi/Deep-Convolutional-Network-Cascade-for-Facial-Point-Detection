# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 09:53:58 2018
...
cache_LM31 = np.zeros([34660,2], dtype = np.float32)
#save the model
...
with tf.Session() as sess:
    ...
    for i in range(151):                                            #iteration times:150*6250=937500, 15 millon images
        ...
        if i % 10 == 0:
            save_path = saver.save(sess, "LM31_net/save_net.ckpt")  #save the model
    ...
    for k in range(17330):
        ...
        cache_LM31[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, y:test_ybatch, keep_prob:1})

#print euclidean distance of the keypoints
printaccuracy = np.mean(testaccuracy, 0)
LM31_accuracy = np.sqrt(np.square(printaccuracy[0])+np.square(printaccuracy[1])) / 39
print('LM31_error_rate is:',LM31_accuracy)
print('output_test_landmarks is:',cache_LM31)
