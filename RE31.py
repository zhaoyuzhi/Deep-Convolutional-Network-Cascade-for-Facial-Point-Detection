...
testaccuracy = np.zeros([17303,2], dtype = np.float32)
cache_RE31 = np.zeros([34660,2], dtype = np.float32)
#save the model
...
with tf.Session() as sess:
    ...
    for i in range(151):                                            #iteration times:150*6250=937500, 15 millon images
        ...
        if i % 10 == 0:
            save_path = saver.save(sess, "RE31_net/save_net.ckpt")  #save the model
...
RE31_accuracy = np.sqrt(np.square(printaccuracy[0])+np.square(printaccuracy[1])) / 39
print('RE31_error_rate is:',RE31_accuracy)
