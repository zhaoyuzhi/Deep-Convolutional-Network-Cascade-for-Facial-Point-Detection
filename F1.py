...
for i in range(1439):                                               #test data part 1
    ...
    #get ten normalized newlandmarks(coordinates of LE,RE,N,LM,RM)
    for j in range(0,9,2):
        newlandmarks[j] = (rawlandmarks[j].value - test_table.cell(i+1,3).value) / width * 39
    for k in range(1,10,2):
        newlandmarks[k] = (rawlandmarks[k].value - test_table.cell(i+1,1).value) / height * 39
    #one dimension which represents one grey picture, set the first dimension as index
...
for i in range(2027):                                               #test data part 2
    ...
    #get ten normalized newlandmarks(coordinates of LE,RE,N,LM,RM)
    for j in range(0,9,2):
        newlandmarks[j] = (rawlandmarks[j].value - test_table.cell(i+1440,3).value) / width * 39
    for k in range(1,10,2):
        newlandmarks[k] = (rawlandmarks[k].value - test_table.cell(i+1440,1).value) / height * 39
    #one dimension which represents one grey picture, set the first dimension as index
