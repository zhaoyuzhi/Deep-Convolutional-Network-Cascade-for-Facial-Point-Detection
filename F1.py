...
'''
## create new dataset, but the result seems not good, I think you should believe and base on the raw dataset
x_data_symmetry = np.zeros([10000,39,39], dtype = np.float32)       #input imagematrix_data
y_data_symmetry = np.ones([10000,10], dtype = np.float32)           #correct output landmarks_data
for i in range(10000):   #1-dimonsion
    for j in range(39):   #row-dimonsion
        for k in range(19):   #column-dimonsion
            x_data_symmetry[i,j,k] = x_data[i,j,38-k]
            x_data_symmetry[i,j,38-k] = x_data[i,j,k]
        x_data_symmetry[i,j,19] = x_data[i,j,19]
    for m in range(0,9,2):
        y_data_symmetry[i,m] = 39 - y_data[i,m]
    for n in range(1,10,2):
        y_data_symmetry[i,n] = y_data[i,n]
'''
...
with tf.Session() as sess:
    ...
    '''
    for n in range(625):
        train_xbatch = x_data_symmetry[(m*16):(m*16+16),:,:]        #train 16 data every batch, not including m*16+16
        train_ybatch = y_data_symmetry[(m*16):(m*16+16),:]          #train 16 data every batch, not including m*16+16
        sess.run(Optimizer, feed_dict = {x:train_xbatch, y:train_ybatch, keep_prob:0.5})
    '''
    for k in range(27):
        test_xbatch = x_test[(k*128):(k*128+128),:,:]               #train 100 data every time, not including m*100+100
