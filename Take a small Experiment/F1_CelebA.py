# -*- coding: utf-8 -*-
"""
Created on Sat Apr 28 15:25:34 2018
...
x_data = np.zeros([202599,39,39], dtype = np.float32)               #input CelebA imagematrix_data
y_data = np.ones([202599,10], dtype = np.float32)                   #correct output landmarks_data
x_test = np.zeros([13466,39,39], dtype = np.float32)                #LFPW and BioID pictures matrix
y_test = np.ones([13466,10], dtype = np.float32)                    #LFPW and BioID pictures landmarks_data
...
for i in range(5849):                                               #train data part 2
    ...
    #one dimension which represents one grey picture, set the first dimension as index
    x_test[i+4151,:,:] = imagematrix
    y_test[i+4151,:] = newlandmarks
...
for i in range(1439):                                               #test data part 1
    ...
    #one dimension which represents one grey picture, set the first dimension as index
    x_test[i+10000,:,:] = imagematrix
    y_test[i+10000,:] = newlandmarks
...
testaccuracy = np.zeros([6733,10], dtype = np.float32)
cache_F1 = np.zeros([13466,10], dtype = np.float32)
#save the model
...
with tf.Session() as sess:
    ...
    for n in range(6733):
        test_xbatch = x_test[(n*2):(n*2+2),:,:]                     #test 2 data every time, not including n*2+2
        test_ybatch = y_test[(n*2):(n*2+2),:]
