...
cache_LM22 = np.zeros([34660,2], dtype = np.float32)
#save the model
...
with tf.Session() as sess:
    ...
    for i in range(151):                                            #iteration times:150*6250=937500, 15 millon images
        ...
        if i % 10 == 0:
            save_path = saver.save(sess, "LM22_net/save_net.ckpt")  #save the model
...
LM22_accuracy = np.sqrt(np.square(printaccuracy[0])+np.square(printaccuracy[1])) / 39
print('LM22_error_rate is:',LM22_accuracy)
