# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 09:33:48 2018
...
for i in range(4151):                                               #train data part 1
    ...
    img = img.convert('L')
    Px = train_table.cell(i+1,13).value                             #x of the ground truth position
    Py = train_table.cell(i+1,14).value                             #y of the ground truth position
    delta_x = math.ceil(0.16*(train_table.cell(i+1,4).value - train_table.cell(i+1,3).value))
    delta_y = math.ceil(0.16*(train_table.cell(i+1,2).value - train_table.cell(i+1,1).value))
    for j in range(10):                                             #as for one same image, increasing the random situation
        rx = -0.3125 + 0.625 * random.random()                      #output random number [-1,1], 0.05 / 0.16 = 0.3125
        ry = -0.3125 + 0.625 * random.random()                      #output random number [-1,1], 0.05 / 0.16 = 0.3125
...
for i in range(5849):                                               #train data part 2
    ...
    img = Image.open(IMAGEURL + '\\' + imagename,'r')               #the whole image
    img = img.convert('L')
    Px = train_table.cell(i+4152,13).value                          #x of the ground truth position
    Py = train_table.cell(i+4152,14).value                          #y of the ground truth position
    delta_x = math.ceil(0.16*(train_table.cell(i+4152,4).value - train_table.cell(i+4152,3).value))
    delta_y = math.ceil(0.16*(train_table.cell(i+4152,2).value - train_table.cell(i+4152,1).value))
    for j in range(10):                                             #as for one same image, increasing the random situation
        rx = -0.3125 + 0.625 * random.random()                      #output random number [-1,1], 0.05 / 0.16 = 0.3125
        ry = -0.3125 + 0.625 * random.random()                      #output random number [-1,1], 0.05 / 0.16 = 0.3125
        newleft = Px + (rx-1) * delta_x                             #level 2 bounding box parameter
...
for i in range(1439):                                               #test data part 1
    ...
    img = Image.open(IMAGEURL + '\\' + imagename,'r')               #the whole image
    img = img.convert('L')
    Px = test_table.cell(i+1,13).value                              #x of the ground truth position
    Py = test_table.cell(i+1,14).value                              #y of the ground truth position
...
for i in range(2027):                                               #test data part 2
    ...
    img = Image.open(IMAGEURL + '\\' + imagename,'r')               #the whole image
    img = img.convert('L')
    Px = test_table.cell(i+1440,13).value                           #x of the ground truth position
    Py = test_table.cell(i+1440,14).value                           #y of the ground truth position
...
a_fc2 = tf.nn.relu(h_fc2)                                           #outsize = batch*2
...
Optimizer = tf.train.AdamOptimizer(0.001).minimize(original_cost)
...
cache_RM21 = np.zeros([34660,2], dtype = np.float32)
#save the model
saver = tf.train.Saver()

with tf.Session() as sess:
    sess.run(init)
    for i in range(151):                                            #iteration times:150*6250=937500, 15 millon images
        for m in range(6250):                                       #training process using training data 100000 images
        ...
        if i % 10 == 0:
            save_path = saver.save(sess, "RM21_net/save_net.ckpt")  #save the model
    ...
    for k in range(17330):
        ...
        cache_RM21[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, y:test_ybatch, keep_prob:1})

#print euclidean distance of the keypoints
printaccuracy = np.mean(testaccuracy, 0)
RM21_accuracy = np.sqrt(np.square(printaccuracy[0])+np.square(printaccuracy[1])) / 39
print('RM21_error_rate is:',RM21_accuracy)
print('output_test_landmarks is:',cache_RM21)
