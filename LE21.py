# -*- coding: utf-8 -*-
"""
Created on Tue Apr 10 20:49:07 2018
...
for i in range(4151):                                               #train data part 1
    ...
    img = img.convert('L')
    Px = train_table.cell(i+1,5).value                              #x of the ground truth position
    Py = train_table.cell(i+1,6).value                              #y of the ground truth position
...
for i in range(5849):                                               #train data part 2
    ...
    img = Image.open(IMAGEURL + '\\' + imagename,'r')               #the whole image
    img = img.convert('L')
    Px = train_table.cell(i+4152,5).value                           #x of the ground truth position
    Py = train_table.cell(i+4152,6).value                           #y of the ground truth position
...
for i in range(1439):                                               #test data part 1
    ...
    img = Image.open(IMAGEURL + '\\' + imagename,'r')               #the whole image
    img = img.convert('L')
    Px = test_table.cell(i+1,5).value                               #x of the ground truth position
    Py = test_table.cell(i+1,6).value                               #y of the ground truth position
...
for i in range(2027):                                               #test data part 2
    ...
    img = Image.open(IMAGEURL + '\\' + imagename,'r')               #the whole image
    img = img.convert('L')
    Px = test_table.cell(i+1440,5).value                            #x of the ground truth position
    Py = test_table.cell(i+1440,6).value                            #y of the ground truth position
...
cache_LE21 = np.zeros([34660,2], dtype = np.float32)
#save the model
...
with tf.Session() as sess:
    ...
    for i in range(151):                                            #iteration times:150*6250=937500, 15 millon images
        ...
        if i % 10 == 0:
            save_path = saver.save(sess, "LE21_net/save_net.ckpt")  #save the model
    ...
    for k in range(17330):
        ...
        cache_LE21[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, y:test_ybatch, keep_prob:1})

#print euclidean distance of the keypoints
printaccuracy = np.mean(testaccuracy, 0)
LE21_accuracy = np.sqrt(np.square(printaccuracy[0])+np.square(printaccuracy[1])) / 39
print('LE21_error_rate is:',LE21_accuracy)
print('output_test_landmarks is:',cache_LE21)
