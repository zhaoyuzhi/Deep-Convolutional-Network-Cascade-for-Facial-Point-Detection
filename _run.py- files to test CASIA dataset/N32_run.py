...
cache_N32 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_N32[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_N32)
## save the predicted keypoints to excel
cache_N32_df = pd.DataFrame(cache_N32)
writer = pd.ExcelWriter('N32.xlsx')
cache_N32_df.to_excel(writer,'sheet1')
writer.save()
