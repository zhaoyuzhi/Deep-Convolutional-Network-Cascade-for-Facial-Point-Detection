...
x_data = np.zeros([10000,28,28], dtype = np.float32)                #input imagematrix_data
y_data = np.ones([10000,10], dtype = np.float32)                    #correct output landmarks_data
x_test = np.zeros([3466,28,28], dtype = np.float32)
y_test = np.ones([3466,10], dtype = np.float32)
CASIA_test = np.zeros([4442,28,28], dtype = np.float32)             #dataset to be processed
...
for i in range(4151):                                               #train data part 1
    ...
    for j in range(0,9,2):
        newlandmarks[j] = (rawlandmarks[j].value - train_table.cell(i+1,3).value) / width * 28
    for k in range(1,10,2):
        newlandmarks[k] = (rawlandmarks[k].value - train_table.cell(i+1,1).value) / height * 28
...
for i in range(5849):                                               #train data part 2
    ...
    #get ten normalized newlandmarks(coordinates of LE,RE,N,LM,RM)
    for j in range(0,9,2):
        newlandmarks[j] = (rawlandmarks[j].value - train_table.cell(i+4152,3).value) / width * 28
    for k in range(1,10,2):
        newlandmarks[k] = (rawlandmarks[k].value - train_table.cell(i+4152,1).value) / height * 28
    #one dimension which represents one grey picture, set the first dimension as index
...
for i in range(1439):                                               #test data part 1
    ...
    #get ten normalized newlandmarks(coordinates of LE,RE,N,LM,RM)
    for j in range(0,9,2):
        newlandmarks[j] = (rawlandmarks[j].value - test_table.cell(i+1,3).value) / width * 28
    for k in range(1,10,2):
        newlandmarks[k] = (rawlandmarks[k].value - test_table.cell(i+1,1).value) / height * 28
    #one dimension which represents one grey picture, set the first dimension as index
...
for i in range(2027):                                               #test data part 2
    ...
    #get ten normalized newlandmarks(coordinates of LE,RE,N,LM,RM)
    for j in range(0,9,2):
        newlandmarks[j] = (rawlandmarks[j].value - test_table.cell(i+1440,3).value) / width * 28
    for k in range(1,10,2):
        newlandmarks[k] = (rawlandmarks[k].value - test_table.cell(i+1440,1).value) / height * 28
    #one dimension which represents one grey picture, set the first dimension as index
...
for i in range(4442):
    ...
    newdata = np.reshape(npdata, (28,28))
...
xs = tf.placeholder(tf.float32, [None,28,28])    #28*28
...
writer = pd.ExcelWriter('test_2conv.xlsx')
