...
for i in range(4442):
    Px = math.ceil(level1_table.cell(i+1,7).value)
    Py = math.ceil(level1_table.cell(i+1,8).value)
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_LM21[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_LM21)
## save the predicted keypoints to excel
cache_LM21_df = pd.DataFrame(cache_LM21)
writer = pd.ExcelWriter('LM21.xlsx')
cache_LM21_df.to_excel(writer,'sheet1')
writer.save()
