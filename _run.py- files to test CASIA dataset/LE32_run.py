...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_LE32[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_LE32)
## save the predicted keypoints to excel
cache_LE32_df = pd.DataFrame(cache_LE32)
writer = pd.ExcelWriter('LE32.xlsx')
cache_LE32_df.to_excel(writer,'sheet1')
writer.save()
