...
level1_excel = xlrd.open_workbook('level1.xlsx')
...
for i in range(4442):
    Px = math.ceil(level1_table.cell(i+1,9).value)
    Py = math.ceil(level1_table.cell(i+1,10).value)
    imagename = CASIA_test_table.cell(i,0).value
    img = Image.open(IMAGECASIA_test + '\\' + imagename,'r')
    delta_x = math.ceil(0.16*100)                                   #CASIA_test is normalized to 100*100 pixel
    delta_y = math.ceil(0.16*100)                                   #CASIA_test is normalized to 100*100 pixel
...
cache_RM21 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_RM21[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_RM21)
## save the predicted keypoints to excel
cache_RM21_df = pd.DataFrame(cache_RM21)
writer = pd.ExcelWriter('RM21.xlsx')
cache_RM21_df.to_excel(writer,'sheet1')
writer.save()
