...
cache_RE22 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_RE22[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_RE22)
## save the predicted keypoints to excel
cache_RE22_df = pd.DataFrame(cache_RE22)
writer = pd.ExcelWriter('RE22.xlsx')
cache_RE22_df.to_excel(writer,'sheet1')
writer.save()
