...
for i in range(4442):
    Px = math.ceil(level1_table.cell(i+1,5).value)
    Py = math.ceil(level1_table.cell(i+1,6).value)
...
cache_N21 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_N21[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_N21)
## save the predicted keypoints to excel
cache_N21_df = pd.DataFrame(cache_N21)
writer = pd.ExcelWriter('N21.xlsx')
cache_N21_df.to_excel(writer,'sheet1')
writer.save()
