...
for i in range(4442):
    ...
    delta_x = math.ceil(0.18*100)                                   #CASIA_test is normalized to 100*100 pixel
    delta_y = math.ceil(0.18*100)                                   #CASIA_test is normalized to 100*100 pixel
...
cache_RM22 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_RM22[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_RM22)
## save the predicted keypoints to excel
cache_RM22_df = pd.DataFrame(cache_RM22)
writer = pd.ExcelWriter('RM22.xlsx')
cache_RM22_df.to_excel(writer,'sheet1')
writer.save()
