...
cache_LM31 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_LM31[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_LM31)
## save the predicted keypoints to excel
cache_LM31_df = pd.DataFrame(cache_LM31)
writer = pd.ExcelWriter('LM31.xlsx')
cache_LM31_df.to_excel(writer,'sheet1')
writer.save()
