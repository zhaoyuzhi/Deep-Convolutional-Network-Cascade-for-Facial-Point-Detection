...
for i in range(4442):
    Px = math.ceil(level2_table.cell(i+1,3).value)
    Py = math.ceil(level2_table.cell(i+1,4).value)
...
cache_RE31 = np.zeros([4442,2], dtype = np.float32)                 #run CASIA_test dataset
...
with tf.Session() as sess:
    ...
    for k in range(2221):
        ...
        cache_RE31[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, keep_prob:1})

print(cache_RE31)
## save the predicted keypoints to excel
cache_RE31_df = pd.DataFrame(cache_RE31)
writer = pd.ExcelWriter('RE31.xlsx')
cache_RE31_df.to_excel(writer,'sheet1')
writer.save()
