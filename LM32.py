# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 10:14:15 2018
...
cache_LM32 = np.zeros([34660,2], dtype = np.float32)
#save the model
...
with tf.Session() as sess:
    ...
    for i in range(151):                                            #iteration times:150*6250=937500, 15 millon images
        ...
        if i % 10 == 0:
            save_path = saver.save(sess, "LM32_net/save_net.ckpt")  #save the model
    ...
    for k in range(17330):
        ...
        cache_LM32[(k*2):(k*2+2),:] = a_fc2.eval(feed_dict = {x:test_xbatch, y:test_ybatch, keep_prob:1})

#print euclidean distance of the keypoints
printaccuracy = np.mean(testaccuracy, 0)
LM32_accuracy = np.sqrt(np.square(printaccuracy[0])+np.square(printaccuracy[1])) / 39
print('LM32_error_rate is:',LM32_accuracy)
print('output_test_landmarks is:',cache_LM32)
